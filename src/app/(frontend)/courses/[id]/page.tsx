...
export default async function CoursePage({ params }: { params: { id: string } }) {
  ...
  // Gather all step IDs from course.relatedTopics?.docs[].relatedSteps?.docs (array of IDs)
  const stepIds: string[] =
    course.relatedTopics?.docs?.flatMap((topic: any) =>
      topic?.relatedSteps?.docs ? topic.relatedSteps.docs : [],
    ) ?? []

  // Fetch all step details in order
  const steps = []
    ...
    <main className="p-8">
      ...
      {steps.length > 0 && (
        <section className="mt-6 space-y-8">
          {steps.map((step, idx) => (
            <article key={step.id || idx} className="border rounded p-4">
              <h2 className="text-xl font-semibold mb-2">{step.title}</h2>
              {step.duration && (
                <div className="text-sm text-gray-500 mb-2">Длительность: {step.duration}</div>
              )}
              {step.content && (
                <div>
                  {renderNode(
